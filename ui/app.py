...
from financial_spreading.extract_rows import extract_rows_from_cu, extract_all_candidates
from financial_spreading.statement_mapper import spread_statement, map_coa_to_document
...
with tab_upload:
    ...
    if selected_blob:
        try:
            ...
            if file_mb > MAX_FILE_MB:
            ...
            elif col2.button("Run analysis", type="primary", key="run_blob"):
                ...
                with st.status("Running analysis pipeline...", expanded=True) as pipeline_status:
                    try:
                        ...
                        # ── Step 3–4/5: Normalize first (financial_rows embedded) ────────
                        progress.progress(35, text="Step 3–4/5 — Saving raw JSON and normalizing…")
                        ...
                        st.write(
                            ...
                            f"Prepared {len(build_index_documents(normalized_document))} chunk(s). ({time.monotonic() - t0:.1f}s)"
                        )

                        # ── Financial Spreading — reads financial_rows from normalized doc ─
                        ...
                        if _doc_type == "tax_document":
                            progress.progress(50, text="Step 2b/5 — Running financial spreading…")
                            st.write("Step 2b/5 — Running financial spreading (COA mapping)…")
                            try:
                                _coa_schema = load_coa_schema(document_type=_doc_type)
                                # Use financial_rows from normalized document — structured
                                # flat candidates extracted from table cells + line fallback
                                _candidates = normalized_document.get("financial_rows", [])
                                _mapped = map_coa_to_document(_candidates, _coa_schema)
                                _matched_labels = {r["source_label"] for r in _mapped}
                                _unmatched = [
                                    {
                                        "label":  c["label"],
                                        "value":  c.get("value"),
                                    ...
                                    if c.get("label") not in _matched_labels
                                ]
                                spread_json = build_spread_output(
                                    ...
                                    document_type=_doc_route.get("document_type", "tax_document"),
                                    document_subtype=_doc_route.get("document_subtype", ""),
                                    schema_id=_doc_route.get("schema_id", ""),
                                    created_utc=created_utc,
                                    source_blob=source_blob,
                                )
                                st.write(
            ...
            if col3.button("Clear", key="clear_blob"):
                for key in [
                    "doc_id",
                    "created_utc",
                    "source_blob",
    ...
    if "raw_json_blob" in st.session_state:
        ...
        try:
            orig_bytes = download_blob_bytes(
                BLOB_INPUT_CONTAINER,
                st.session_state["source_blob"],
            )
            orig_file_name = st.session_state.get("file_name", "document.pdf")
            st.download_button(
            ...
            st.warning(f"Could not download original document: {exc}")
            ...
            if "year" in df_display.columns:
                def _fmt_year(v):
                    ...
                    try:
                        ...
                        return s
                df_display["year"] = df_display["year"].apply(_fmt_year)
...
with tab_ask:
    if st.session_state.get("normalized_document"):
        ...
        with st.form(key="doc_qa_form", border=False):
            ...
            with col_btn:
                doc_ask = st.form_submit_button("Ask", type="primary")
