...
def _is_financial_amount(original_text: str, parsed_value: float) -> bool:
    ...
    # Reject 9-digit integers that look like SSNs (no formatting: no commas,
    # no decimal, no dollar sign, no parentheses).  Real financial amounts at
    # that scale always use comma formatting in tax documents.
    t_stripped = original_text.strip()
    if t_stripped.isdigit() and len(t_stripped) == 9:
        return False

    # Keep ALL numeric values (critical for tax documents)
    return True
 
# Words that, when a label ends with them, signal the line was wrapped
# mid-phrase by the OCR engine.  Used by both table-cell and line-fallback
# extraction paths to stitch split labels back together.
_CONNECTOR_ENDINGS: frozenset = frozenset({
    "see", "the", "a", "an", "and", "or", "of", "for", "to", "in",
    "on", "at", "from", "with", "by", "per", "as", "if", "that",
    "this", "these", "those", "such", "any", "each", "both",
    "but", "not", "less", "more", "than", "after", "before",
    "under", "over", "into", "upon",
})


def _label_ends_with_connector(label: str) -> bool:
    """Return True if the label ends with a word that implies a wrapped line."""
    words = label.strip().rstrip(".,;:").split()
    return bool(words) and words[-1].lower() in _CONNECTOR_ENDINGS


# Words that appear in tax-form dependent/personal tables but are never
# valid financial account labels.  Matched case-insensitively.
_NON_FINANCIAL_LABELS: set = {
    "daughter", "son", "spouse", "dependent", "self",
    "wife", "husband", "parent", "father", "mother",
    "brother", "sister", "child", "grandchild", "nephew",
    "niece", "foster child", "stepchild",
    "qualifying child", "qualifying relative",
    "yes", "no", "n/a", "na",
    # OCR checkbox markers
    ":selected:", ":unselected:",
}

# IRS boilerplate fragments that are NEVER standalone financial labels but ARE
# valid continuation fragments when stitched onto a longer label (e.g. "See" +
# "instructions"). Filtered AFTER label stitching in extract_all_candidates.
_BOILERPLATE_STANDALONE_LABELS: frozenset = frozenset({
...
def _is_valid_financial_label(label: str) -> bool:
    ...
    - personal relationship words (Daughter, Son, Spouse ...)

    This ensures we do NOT treat reference markers or dependent-table
    relationship cells as financial labels.
    """
    ...
    # Reject personal/relationship words that appear in dependent tables
    if norm in _NON_FINANCIAL_LABELS:
        return False
...
def _rows_from_cell_group(
    ...
    # Stash a connector-ending label that had no value in its row, so the
    # next row's short continuation text can be prepended to it.
    # E.g. Row N:   "Taxable income. Subtract line 29c from line 28. See"  (no value)
    #      Row N+1: "instructions"  456,123  → stitched label = full sentence
    stashed_label: Optional[str] = None
    stashed_label_conf: float = 0.0
    ...
    for row_idx in sorted(row_map.keys()):
        ...
        if use_x_layout:
            ...
            for col_idx, cell_text in ordered:
                if (
                    ...
                    and _is_valid_financial_label(cell_text)
                ):
            ...
            for col_idx, cell_text in ordered:
                ...
                if parsed is None:
                    continue
                if not _is_financial_amount(cell_text.strip(), parsed):
                    continue
            ...
            if not all_values:
                # Stash if label ends with a connector word (mid-phrase wrap)
                if all_labels and _label_ends_with_connector(all_labels[0][1]):
                    stashed_label = all_labels[0][1]
                    stashed_label_conf = conf_map.get(row_idx, {}).get(all_labels[0][0], 0.0)
                else:
                    stashed_label = None
                continue
            ...
            if has_periods or len(all_labels) <= 1:
                ...
                # Prepend stashed connector label when this looks like a continuation
                lbl_conf = conf_map.get(row_idx, {}).get(lbl_col, 0.0)
                if stashed_label is not None and len(lbl_text.split()) <= 4:
                    lbl_text = stashed_label + " " + lbl_text
                    lbl_conf = stashed_label_conf
                    stashed_label = None
                rows.append({
                    "label": lbl_text,
                    "values": all_values,
        ...
        for half_ordered in halves:
            ...
            if values:
                ...
                # Prepend stashed connector label when this looks like a continuation
                if stashed_label is not None and len(label.split()) <= 4:
                    label = stashed_label + " " + label
                    label_conf = stashed_label_conf
                    stashed_label = None
            ...
            elif _label_ends_with_connector(label):
                # No values but connector-ending label — stash for next row
                stashed_label = label
                stashed_label_conf = label_conf
...
def _rows_from_lines_fallback(
    ...
    while i < len(lines):
        ...
        values: List[Dict[str, Any]] = []
        j = i + 1
        label_extended = False  # allow at most one continuation stitch per label
        while j < len(lines) and j <= i + 7:
            ...
            if parsed is not None:
                ...
                if not next_text or _is_line_reference_cell(next_text) or _is_unusable_label(next_text):
                    j += 1
                    continue
                # Stitch parenthesized IRS annotations onto the label so the
                # full label is preserved, e.g.:
                ...
                if next_text.startswith("(") and next_text.endswith(")"):
                    ...
                    j += 1
                    continue
                # Stitch wrapped label fragments: if the current label ends with
                # a connector word (e.g. "See") and this line is a short fragment
                # (e.g. "instructions"), append it to the label and keep scanning.
                if not label_extended and _label_ends_with_connector(text):
                    text = text + " " + next_text
                    label_extended = True
                    j += 1
                    continue
    ...
    for key in sorted(table_groups.keys()):
        for row in _rows_from_cell_group(table_groups[key]):
            ...
            if not _is_valid_financial_label(label):
                continue
            if label.strip().lower() in _BOILERPLATE_STANDALONE_LABELS:
                continue
...
# ---------------------------------------------------------------------------
# Dependent extraction (Form 1040 Schedule for dependents)
# ---------------------------------------------------------------------------

# Relationship words that identify a line as a dependent's relationship field.
_DEPENDENT_RELATIONSHIP_WORDS: set = {
    "daughter", "son", "spouse", "dependent", "self",
    "wife", "husband", "parent", "father", "mother",
    "brother", "sister", "child", "grandchild", "nephew",
    "niece", "foster child", "stepchild",
...
# Capitalized full name: two or more words, each starting with a capital letter.
_NAME_RE = re.compile(r"^[A-Z][a-zA-Z'-]+(?: [A-Z][a-zA-Z'-]+)+$")


def extract_dependents_from_cu(cu_json: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Extract Form 1040 dependent records from the Azure Content Understanding JSON.

    Strategy: anchors on bare 9-digit SSN lines (the most distinctive marker),
    then looks back up to 2 lines for the dependent's name and forward up to
    2 lines for the relationship word.

    Returns a list of dicts:
        {
          "dependent_name": "Sally Garcia",
          "ssn":            "450359708",
    ...
    for i, line in enumerate(lines):
        ...
        # Anchor: bare 9-digit integer that looks like a SSN
        if not (text.isdigit() and len(text) == 9):
            continue
        ssn = text
        if ssn in seen_ssns:
            continue

        # Look back up to 2 lines for a capitalized full name
        name = ""
        for back in range(1, 3):
            if i - back < 0:
                break
            candidate = lines[i - back].get("text", "").strip()
            if _NAME_RE.match(candidate):
                name = candidate
                break

        if not name:
            continue  # No name found — skip (not a dependent entry)

        # Look forward up to 2 lines for a relationship word
        relationship = ""
        for fwd in range(1, 3):
            if i + fwd >= len(lines):
                break
            candidate = lines[i + fwd].get("text", "").strip()
            if candidate.lower() in _DEPENDENT_RELATIONSHIP_WORDS:
